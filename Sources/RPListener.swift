...
open class RPListener: NSObject, XCTestObservation {
    ...
    private func readConfiguration(from testBundle: Bundle) -> AgentConfiguration {
        guard
            ...
            let logLevel = bundleProperties["LogLevel"] as? String,
        ...
        if let ll = Loger.LogLevel(rawValue: logLevel.lowercased()) {
            Loger.logLevel = ll
        }
        ...
        public func testBundleWillStart(_ testBundle: Bundle) {
            ...
            guard configuration.shouldSendReport else {
                Loger.print("Set 'YES' for 'PushTestDataToReportPortal' property in Info.plist if you want to put data to report portal")
                return
            }
            ...
            queue.async {
                do {
                    ...
                    Loger.print("🚨 RPListener Launch Start Error: Failed to start ReportPortal launch for test bundle. This will prevent all test results from being reported. Error details: \(error.localizedDescription)")
                }
            }
        }
        ...
        public func testSuiteWillStart(_ testSuite: XCTestSuite) {
            guard let reportingService = self.reportingService else {
                Loger.print("🚨 RPListener Configuration Error: ReportingService is not available. Test suite '\(testSuite.name)' will not be reported to ReportPortal.")
                return
            }
            ...
            queue.async {
                do {
                    ...
                    Loger.print("🚨 RPListener Suite Start Error: Failed to start test suite '\(testSuite.name)' in ReportPortal. Error details: \(error.localizedDescription)")
                }
            }
        }
        ...
        public func testCaseWillStart(_ testCase: XCTestCase) {
            guard let reportingService = self.reportingService else {
                Loger.print("🚨 RPListener Configuration Error: ReportingService is not available. Test case '\(testCase.name)' will not be reported to ReportPortal.")
                return
            }
            ...
            queue.async {
                do {
                    ...
                    Loger.print("🚨 RPListener Test Start Error: Failed to start test case '\(testCase.name)' in ReportPortal. Error details: \(error.localizedDescription)")
                }
            }
        }
        ...
        public func testCase(_ testCase: XCTestCase, didRecord issue: XCTIssueReference) {
            guard let reportingService = self.reportingService else {
                Loger.print("🚨 RPListener Configuration Error: ReportingService is not available. Test issue for '\(testCase.name)' will not be reported to ReportPortal.")
                return
            }
            ...
            queue.async {
                do {
                    ...
                    Loger.print("🚨 RPListener Issue Reporting Error: Failed to report test issue for '\(testCase.name)' to ReportPortal. Error details: \(error.localizedDescription)")
                }
            }
        }
        ...
        public func testCase(_ testCase: XCTestCase, didFailWithDescription description: String, inFile filePath: String?, atLine lineNumber: Int) {
            guard let reportingService = self.reportingService else {
                Loger.print("🚨 RPListener Configuration Error: ReportingService is not available. Test failure for '\(testCase.name)' will not be reported to ReportPortal.")
                return
            }
            ...
            queue.async {
                do {
                    ...
                    Loger.print("🚨 RPListener Failure Reporting Error: Failed to report test failure for '\(testCase.name)' to ReportPortal. Error details: \(error.localizedDescription)")
                }
            }
        }
        ...
        public func testCaseDidFinish(_ testCase: XCTestCase) {
            guard let reportingService = self.reportingService else {
                Loger.print("🚨 RPListener Configuration Error: ReportingService is not available. Test completion for '\(testCase.name)' will not be reported to ReportPortal.")
                return
            }
            ...
            queue.async {
                do {
                    ...
                    Loger.print("🚨 RPListener Test Finish Error: Failed to finish test case '\(testCase.name)' in ReportPortal. Error details: \(error.localizedDescription)")
                }
            }
        }
        ...
        public func testSuiteDidFinish(_ testSuite: XCTestSuite) {
            ...
            queue.async {
                do {
                    ...
                    Loger.print("🚨 RPListener Suite Finish Error: Failed to finish test suite '\(testSuite.name)' in ReportPortal. Error details: \(error.localizedDescription)")
                }
            }
        }
        ...
        public func testBundleDidFinish(_ testBundle: Bundle) {
            ...
            queue.sync() {
                do {
                    ...
                    Loger.print("🚨 RPListener Launch Finish Error: Failed to finish ReportPortal launch for test bundle. Error details: \(error.localizedDescription)")
                }
            }
        }
    }
